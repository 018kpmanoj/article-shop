...
export const registerProductInterest = async (productId, userEmail, userName = '') => {
  ...
  try {
    ...
    // Check if already interested - simple query without compound index requirement
    const existingQuery = query(
      collection(db, 'product_interests'),
      where('productId', '==', productId)
    );
    
    const existing = await Promise.race([getDocs(existingQuery), timeoutPromise]);
    
    // Check if user email exists in results
    const alreadyExists = existing.docs.some(doc => doc.data().email === normalizedEmail);
    ...
    // Add interest
    await Promise.race([
      addDoc(collection(db, 'product_interests'), {
        productId,
